...
export default function RegisterPage() {
  ...
  const next = (e) => { e.preventDefault(); setDir(1); setStep(2); };
  const back = () => { setDir(-1); setStep(1); };
  ...
  return (
    <div className="min-h-screen bg-[#18120a] text-[#eddcba] flex overflow-hidden">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-5">
            {[
              ...
              <motion.div key={n} className="flex items-center gap-4" animate={{ opacity: step >= n ? 1 : 0.35 }} transition={{ duration: 0.3 }}>
                <motion.div
                  className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[12px] font-bold flex-shrink-0 transition-colors duration-300 ${
                    step > n ? 'border-amber-500 bg-amber-500 text-[#18120a]'
                    : step === n ? 'border-amber-500 text-amber-400'
                ...
                <div>
                  <div className={`text-[13px] font-semibold transition-colors duration-300 ${step >= n ? 'text-[#eddcba]' : 'text-stone-600'}`}>{label}</div>
                  <div className="text-[11px] text-stone-600">{sub}</div>
                </div>
                {step === n && (
                  <motion.div className="w-1.5 h-1.5 rounded-full bg-amber-500 ml-auto"
        ...
        <div className="relative z-10 text-stone-700 text-[11px] font-mono">Данные защищены · GDPR</div>
      </motion.div>
      ...
      >
        <motion.button onClick={() => navigate('/')} className="flex lg:hidden items-center gap-2 text-amber-500 mb-10">
          <GiYarn />
          <span className="font-black tracking-[0.2em] text-[13px] uppercase">ShveAI</span>
        </motion.button>

        <div className="w-full max-w-[400px]">
          <motion.div className="mb-7" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}>
            ...
            <p className="text-stone-500 text-[13px]">
              Уже есть аккаунт?{' '}
              <Link to="/login" className="text-amber-500 hover:text-amber-400 transition-colors underline underline-offset-2">Войти</Link>
            </p>
          </motion.div>
          ...
          <div className="overflow-hidden">
            <AnimatePresence mode="wait" custom={dir}>
              {step === 1 && (
                <motion.form key="step1" custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit" onSubmit={next} className="space-y-4">
                  {/* Name */}
                  <div>
                    <label className="block text-[11px] text-stone-500 uppercase tracking-[0.15em] mb-2 font-mono">Полное имя</label>
                    <motion.div className={inputWrap('name')} animate={focused === 'name' ? { scale: 1.01 } : { scale: 1 }} transition={{ duration: 0.15 }}>
                      <input type="text" value={form.name} onChange={e => set('name', e.target.value)}
                        onFocus={() => setFocused('name')} onBlur={() => setFocused('')}
                        placeholder="Иванова Елена" className={inputClass} required />
                    </motion.div>
                  </div>
                  ...
                  <div>
                    ...
                    <motion.div className={inputWrap('email')} animate={focused === 'email' ? { scale: 1.01 } : { scale: 1 }} transition={{ duration: 0.15 }}>
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-stone-600 text-[13px] font-mono">@</span>
                      <input type="email" value={form.email} onChange={e => set('email', e.target.value)}
                        onFocus={() => setFocused('email')} onBlur={() => setFocused('')}
                        placeholder="ivanova@factory.ru" className={`${inputClass} pl-9`} required />
                    </motion.div>
                  </div>
                  ...
                  <div>
                    ...
                    <motion.div className={inputWrap('password')} animate={focused === 'password' ? { scale: 1.01 } : { scale: 1 }} transition={{ duration: 0.15 }}>
                      <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => set('password', e.target.value)}
                        onFocus={() => setFocused('password')} onBlur={() => setFocused('')}
                        placeholder="Минимум 8 символов" className={`${inputClass} pr-12`} required />
                    ...
                    <AnimatePresence>
                      {form.password && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-2 overflow-hidden">
                          ...
                          <div className="text-[11px] font-mono" style={{ color: strengthColor[strength] }}>{strengthLabel[strength]}</div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
                  ...
                  <div>
                    ...
                    <motion.div
                      ...
                      animate={focused === 'confirm' ? { scale: 1.01 } : { scale: 1 }} transition={{ duration: 0.15 }}
                    >
                      <input type="password" value={form.confirm} onChange={e => set('confirm', e.target.value)}
                        onFocus={() => setFocused('confirm')} onBlur={() => setFocused('')}
                        placeholder="••••••••" className={`${inputClass} pr-10`} required />
                      <AnimatePresence>
                        {form.confirm && (
                  ...
                  <div className="flex items-center gap-3 text-stone-700">
                    ...
                    <span className="text-[11px] font-mono">или через</span>
                    <div className="flex-1 h-px bg-[#2e2010]" />
                  </div>

                  {/* OAuth */}
                  <motion.button type="button" onClick={() => handleOAuth('google')} disabled={!!oauthLoading}
              ...
              {step === 2 && (
                <motion.form key="step2" custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit" onSubmit={submit} className="space-y-5">
                  {/* Role */}
                  <div>
                    <label className="block text-[11px] text-stone-500 uppercase tracking-[0.15em] mb-3 font-mono">Ваша роль</label>
                    <div className="space-y-2">
                      {ROLES.map((r) => (
                        <motion.button key={r.id} type="button" onClick={() => set('role', r.id)}
                          ...
                          className={`w-full flex items-center gap-4 p-4 rounded-xl border text-left transition-colors duration-200 ${
                            form.role === r.id ? 'border-amber-600/60 bg-amber-950/25' : 'border-[#2e2010] bg-[#1f1610] hover:border-amber-900/50'
                          }`}>
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors duration-200 ${form.role === r.id ? 'bg-amber-600/20' : 'bg-[#2e2010]'}`}>
                            <r.icon className={`text-lg transition-colors duration-200 ${form.role === r.id ? 'text-amber-400' : 'text-stone-600'}`} />
                          </div>
                          <div className="flex-1">
                            <div className={`text-[14px] font-semibold transition-colors duration-200 ${form.role === r.id ? 'text-amber-400' : 'text-[#eddcba]'}`}>{r.label}</div>
                            <div className="text-[12px] text-stone-600">{r.desc}</div>
                          </div>
                          <motion.div
                            className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors duration-200 ${form.role === r.id ? 'border-amber-500' : 'border-[#3a2a15]'}`}
                            animate={form.role === r.id ? { scale: [1, 1.2, 1] } : {}} transition={{ duration: 0.2 }}
                          >
                  ...
                  <div>
                    ...
                    <motion.div className={inputWrap('factory')} animate={focused === 'factory' ? { scale: 1.01 } : { scale: 1 }} transition={{ duration: 0.15 }}>
                      <input type="text" value={form.factory} onChange={e => set('factory', e.target.value)}
                        onFocus={() => setFocused('factory')} onBlur={() => setFocused('')}
                        placeholder="ООО «ТекстильПром»" className={inputClass} />
                    </motion.div>
                  </div>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-stone-500 text-[13px] leading-relaxed">
                      Принимаю{' '}
                      <span className="text-amber-700 underline underline-offset-2 cursor-pointer hover:text-amber-500 transition-colors">условия использования</span>
                    </span>
                  </label>
                  ...
                  <div className="flex gap-2">
                    <motion.button type="button" onClick={back} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                      ...
                      <TbArrowLeft size={15} />Назад
                    </motion.button>
                    <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.97 }}
                      className="flex-1 py-3.5 bg-amber-600 text-[#18120a] font-bold rounded-xl hover:bg-amber-500 transition-colors shadow-[0_8px_24px_-6px_rgba(180,120,40,0.4)] text-[15px] flex items-center justify-center gap-2 overflow-hidden disabled:opacity-70">
                          ...
                          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex gap-1">
                            {[0, 1, 2].map(i => (
                              <motion.div key={i} className="w-1.5 h-1.5 bg-[#18120a] rounded-full"
                                animate={{ y: [0, -5, 0] }} transition={{ duration: 0.6, delay: i * 0.12, repeat: Infinity }} />
                            ))}
                          </motion.div>
                        ) : (
                          <motion.span key="text" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2">
                            <TbCheck size={16} />Создать аккаунт
                          </motion.span>
                        )}
                      </AnimatePresence>
                    </motion.button>
          ...
          <div className="mt-8 pt-5 border-t border-[#2e2010] flex items-center justify-between text-stone-700 text-[11px] font-mono">
            ...
            <div className="flex gap-1">
              {[1, 2].map(n => (
                <motion.div key={n} className="h-1 rounded-full bg-[#2e2010] overflow-hidden" animate={{ width: step >= n ? 16 : 6 }} transition={{ duration: 0.3 }}>
